...
def upload_one(photo, mime):
    ...
    photo = PhotoModels(name=filename, href=file_url, mime=mime, size=size)
...
def delete_photo_by_id(_id):
    photo_name = PhotoModels.query.filter_by(id=_id).first().name
    photo = PhotoModels.query.filter_by(id=_id).delete()
    db.session.commit()
    upload_url = current_app.config.get("UPLOADED_PHOTOS_DEST")
