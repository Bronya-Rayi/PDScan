...
import logging
from re import A
from models import TaskModels, DomainModels, IPModels, SiteModels
...
class BackgroundTaskManager(object):
    ...
    def startBackgroundTask(self):
        # 判断是否已经开启了分配任务的子线程
        if self.assign_thread_flag:
            logger.info('后台任务已经开始')
        else:
            logger.info("后台任务正在启动")
            self.assign_thread_flag = True
            self.assign_thread = threading.Thread(target=self.assignTasks)
    ...
    def assignTasks(self):
        ...
        while self.assign_thread_flag == True:
            # 准备子域名任务
            if self.subdomain_thread_flag:
            ...
            # 准备端口扫描任务
            if self.portscan_thread_flag == True:
            ...
            # 准备网站查找任务
            if self.webfind_thread_flag == True:
            ...
            # 准备漏洞扫描任务
            if self.vulscan_thread_flag == True:
    ...
    def startSubdomain(self,domain_task):
        try:
            ...
            for domain in domain_list:
                ...
                cmd_result = task_manage.out_file_cmd_exec(config.ONEFORALL_CMD.format(target_domain=domain), config.ONEFORALL_LOG_PATH)
                if cmd_result == 0:
    ...
    def startPortscan(self,portscan_task):
        ...
        try:
            ...
            # print(ip_list)
            # 清除端口扫描的日志
            os.system("echo '' > {}".format(config.SCANINFO_LOG_PATH))
            logger.info("任务：{} 的C段整理和ip去重工作完成，开始端口扫描".format(portscan_task.task_name))
            result_file = os.path.join(config.SCANINFO_RESULTS_PATH, portscan_task.task_id)
            for ip in ip_list:
                # 开始端口扫描任务
                # 清空结果文件
                ...
                # print(cmd)
                cmd_result = task_manage.out_file_cmd_exec(cmd, config.SCANINFO_LOG_PATH)
                # cmd_result = 0
                if cmd_result == 0:
    ...
    def startWebFind(self, webfind_task):
        ...
        self.vulscan = webfind_task.task_vulscan
        try:
            # 开始网站查找任务
            ...
            if subdomain_db.count() > 0:
                ...
                cmd_result = task_manage.out_file_cmd_exec(cmd, config.HTTPX_LOG_PATH)
                if cmd_result == 0:
    ...
    def startVulscan(self, vulscan_task):
        ...
        xray_thread = threading.Thread(target=task_manage.out_file_cmd_exec, args=(cmd, config.XRAY_LOG_PATH,))
        xray_thread.start()
        # 将xray的结果文件目录添加至数据库
        task_xray_result = "/static/xray_results/" + result_file_name
        ...
        try:
            ...
            for site in site_db:
                ...
                result = task_manage.out_file_cmd_exec(cmd, config.CRAWLERGO_LOG_PATH)
                if result == 0:
