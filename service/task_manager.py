...
from common.utils import task_manage
...
class TaskManager(object):
    def __init__(self,task_id, task_name,task_target, task_need_run_module=['subdomain','portscan','website_analysis']):
        self.task_id = task_id
        self.task_name = task_name
        self.task_target_list = json.loads(task_target)
        self.task_need_run_module = task_need_run_module
    ...
    def stopTask(task_id):
        ...
        if task_running_module == 'subdomain':
            current_app.BackgroundTask.stopSubdomain()
        elif task_running_module == 'portscan':
            current_app.BackgroundTask.stopPortscan()
        elif task_running_module == 'webfind':
            current_app.BackgroundTask.stopWebfind()
        elif task_running_module == 'vulscan':
            current_app.BackgroundTask.stopVulscan()
    ...
    def checkIsDoaminOrIp(self):
        ...
        TaskModels.query.filter_by(task_id=self.task_id).update({'task_target_domain': json.dumps(self.task_domain),'task_target_ip': json.dumps(self.task_ip)})
        db.session.commit()
    ...
    def startSubdomainTask(self):
        ...
        '''
        # 先查询是否有子域名任务正在运行
        task_info = TaskModels.query.filter_by(task_running_module='subdomain',task_status='Running').first()
        if task_info:
            logger.info("任务：%s 进入子域名查询队列" % self.task_name)
            # 先更新任务模组为子域名查询，然后任务状态为等待
            TaskModels.query.filter_by(task_id=self.task_id).update({'task_running_module': 'subdomain','task_status': 'Waiting'})
        else:
            # 更新任务模组为自域名查询，状态为运行中
            logger.info("任务：%s 开始进行子域名查询" % self.task_name)
            TaskModels.query.filter_by(task_id=self.task_id).update({'task_running_module': 'subdomain','task_status': 'Running'})
        db.session.commit()
    ...
    def startPortScanTask(self):
        ...
        '''
        # 先查询是否有端口扫描任务正在运行
        task_info = TaskModels.query.filter_by(task_running_module='portscan',task_status='Running').first()
        if task_info:
            logger.info("任务：%s 进入端口扫描队列" % self.task_name)
            # 先更新任务模组为端口扫描，然后任务状态为等待
            TaskModels.query.filter_by(task_id=self.task_id).update({'task_running_module': 'portscan','task_status': 'Waiting'})
        else:
            # 更新任务模组为端口扫描，状态为运行中
            logger.info("任务：%s 开始进行端口扫描" % self.task_name)
            TaskModels.query.filter_by(task_id=self.task_id).update({'task_running_module': 'portscan','task_status': 'Running'})
        db.session.commit()