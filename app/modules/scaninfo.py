...
from app.models import TaskModels, DomainModels, IPModels, SiteModels
...
def scaninfo_module(task_id):
    ...
    try:
        scan_ports = config.SCAN_PORTS[scaninfo_task.task_port_limit]
    except:
        scan_ports = scaninfo_task.task_port_limit

    print("[+] 任务：{} 开始进行C段整理".format(task_id))
    ip_list = json.loads(scaninfo_task.task_target_ip)
    c_duan = []
    # 将目标ip加入c段列表
    for ip in ip_list:
        # print(ip)
    ...
    for ip in ip_list:
        ...
        if cmd_result == 0:
            ...
            print("[!] IP：{} 的scaninfo模块失败，scaninfo模块期间出现错误，subprocess返回值不为0".format(ip))
            continue
