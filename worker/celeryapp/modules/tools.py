...
def out_file_cmd_exec(cmd, file_path='log/tool.log'):
    ...
    with open(file_path, 'w') as f:
        print(file_path)
        print(cmd)
        p = subprocess.Popen(cmd, stdout=f, stderr=f,shell=True).wait()
        return p
...
def kill_dirsearch():
    os.system(r"ps -ef |grep dirsearch |grep -v grep |awk '{print $2}'|xargs kill -9")
